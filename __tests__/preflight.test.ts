...
describe('preflightCapture', () => {
  it.each([
    ['SECRET', { scope: 'UNCATEGORIZED', text: 'password: example' }],
    ['SENSITIVE', { scope: 'UNCATEGORIZED', text: '診断内容を確認する' }],
  ...
  it('accepts only a bounded low-risk input as a server-review candidate', () => {
    expect(preflightCapture({ scope: 'UNCATEGORIZED', text: 'renew the library card' })).toEqual({
