...
begin
  ...
  if not found or (select status from public.consents where account_id = v_account_id and consent_type = 'P1_CORE' order by occurred_at desc limit 1) <> 'ACCEPTED' then
  ...
  values (v_account_id, p_auth_user_id::text, case when p_sensitivity_class = 'SECRET' then 'CAPTURE_REJECTED' else 'CAPTURE_STORED' end, 'capture', v_capture_id);
