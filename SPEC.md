...
- **D10 / `attention-client`**: ADR-010をAcceptedとする。In-app確認を全ACTの正規かつ必須経路、Pushを補助チャネルとし、明示的opt-in・iOS authorized permission・active consent・ACTIVE token・送信時再検証を資格条件にする。provisional/ephemeral permissionは除外し、submission retryなし・15分後のticket単回reconcile、missing/error receiptは`RECEIPT_ERROR`、`DeviceNotRegistered`はinstallation無効化とする。opt-in撤回・logout・同意撤回・Account削除・token更新後は旧installationを使わない。Raw/PIIなしpayload、推測不能なopaque参照、stale抑止、permission拒否・provider障害時のIn-app fallbackを実装・実機証跡のrelease gateとする。
