...
- **D11 / `privacy-ops`**: 削除・Raw保持・アカウント削除・sanitized telemetryの境界はADR-011のサブ決定で固定した。アプリはRaw等のバックアップ／エクスポートを作らず、Rawのディスクキャッシュを持たない。Supabase等の事業者バックアップ保持・削除反映・復元時の扱いは未決で、公式条件とDPAの確認・承認まで参加者データを送らない。
