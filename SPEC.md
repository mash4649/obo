...
- **D10 / `attention-client`**: ADR-010をAcceptedとする。In-app確認を全ACTの必須経路、Pushを補助経路とし、明示的opt-in・iOS authorized permission・active consent・ACTIVE token・送信時再検証を資格条件にする。provisional/ephemeral permissionは除外し、submission retryなし・15分後のticket単回reconcile、missing/error receiptは`RECEIPT_ERROR`、`DeviceNotRegistered`はinstallation無効化とする。Raw/PIIなしpayload、stale抑止、permission拒否・provider障害時のIn-app fallbackを実装・実機証跡のrelease gateとする。
