...
- D11（2026-09-15作業提案）: Raw削除、作成から最大7日、Account削除、再識別不能な集計値のみ保持、Supabase sanitized telemetry、外部crash/session replay/analytics SDKなしをADR-011に記録した。P1アプリはRaw等のバックアップ／エクスポートと永続キャッシュを作らず、Rawのディスクキャッシュも持たない。事業者バックアップの保持・削除反映・復元時の扱いは公式条件とDPA確認まで未決で、参加者データ開始を止める。
